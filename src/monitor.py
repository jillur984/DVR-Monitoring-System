...
import telegram_bot
from config import get_dvrs, REFRESH_TIME, load_telegram_config
...
def check_all_dvrs_concurrently(dvrs_list=None, max_workers=30):
    ...
    process_telegram_alerts(results)
    return results
...
def process_telegram_alerts(results):
    ...
    try:
        ...
        for dvr in results:
            ...
            prev = PREVIOUS_STATES.get(ip)
            curr_online = dvr.get("online", False)
            curr_hdd_cnt = dvr.get("hdd_count", 0)
            curr_hdd_status = dvr.get("hdd_status", "")

            if prev is not None:
                prev_online = prev.get("online", False)
                prev_hdd_cnt = prev.get("hdd_count", 0)

                # Offline Alert
                if notify_offline and prev_online and not curr_online:
                    msg = telegram_bot.format_offline_alert(dvr)
                    telegram_bot.send_telegram_message(bot_token, chat_id, msg)

                # Recovery Alert
                if notify_offline and not prev_online and curr_online:
                    msg = telegram_bot.format_recovery_alert(dvr)
                    telegram_bot.send_telegram_message(bot_token, chat_id, msg)

                # HDD Issue Alert
                if notify_hdd and curr_online:
                    if (curr_hdd_cnt == 0 or "ERROR" in str(curr_hdd_status).upper()) and (prev_hdd_cnt > 0 or prev.get("hdd_status") != curr_hdd_status):
                        msg = telegram_bot.format_hdd_alert(dvr)
                        telegram_bot.send_telegram_message(bot_token, chat_id, msg)
            else:
                # First scan detection for offline DVR
                if notify_offline and not curr_online:
                    msg = telegram_bot.format_offline_alert(dvr)
                    telegram_bot.send_telegram_message(bot_token, chat_id, msg)
            ...
            PREVIOUS_STATES[ip] = {
                "online": curr_online,
                "hdd_count": curr_hdd_cnt,
                "hdd_status": curr_hdd_status
            }

        # Periodic Summary
        now = time.time()
        if notify_summary and (LAST_SUMMARY_TIME == 0 or (now - LAST_SUMMARY_TIME >= summary_interval)):
